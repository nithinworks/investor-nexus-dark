        ...
        <div className="relative">
          <Avatar className="h-14 w-14 border-2 border-white/20 ring-2 ring-red-500/20 flex-shrink-0">
            ...
            <AvatarFallback className="bg-gradient-to-br from-red-500/20 to-red-600/20 backdrop-blur-sm border border-white/10 relative">
              ...
              <span className="relative text-white text-sm font-semibold font-satoshi z-10">
                {investor.name
                  .split(" ")
                  .map((n) => n[0])
                  .join("")
          ...
          <div className="flex items-start justify-between">
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 mb-1">
                <h3 className="text-lg font-semibold text-white truncate font-satoshi">
                  {investor.name}
                </h3>
                {investor.verified && (
                  <div className="bg-red-500/20 rounded-full p-1 border border-red-500/30 backdrop-blur-sm">
                    <CheckCircle className="h-3 w-3 text-red-400" />
                  </div>
                )}
              </div>
              
              <div className="flex items-center gap-3 text-sm text-gray-400 mb-2 font-satoshi">
                {investor.company && (
              ...
              {investor.funding_description && (
                <p className="text-xs text-gray-500 line-clamp-2 max-w-md font-satoshi">
                  {investor.funding_description}
                </p>
              )}
            </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {canSave && onToggleSave && (
            <Button
              ...
              className={`h-9 w-9 rounded-full transition-all duration-200 backdrop-blur-sm border font-satoshi ${
                isSaved
          ...
          <Button
            ...
            className="bg-white/10 hover:bg-white/20 text-white border border-white/20 hover:border-white/30 backdrop-blur-sm text-sm px-4 h-9 transition-all duration-200 font-semibold font-satoshi shadow-lg hover:shadow-white/10"
          >
            View Profile
          </Button>
        </div>
